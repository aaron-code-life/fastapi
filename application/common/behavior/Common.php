...
class Common
{
    ...
    public function modelValidate(&$model,$extra){
        //是否采用模型验证
        if ($extra['is_validate']) {
            $name = str_replace("\\model\\", "\\validate\\", get_class($this->model));
            $validate = is_bool($this->modelValidate) ? ($this->modelSceneValidate ? $name . $extra['is_validate'] : $name) : $this->modelValidate;

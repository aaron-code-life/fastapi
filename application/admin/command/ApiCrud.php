...
class ApiCrud extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        try {
            ...
            $recyclebinHtml = '';
            //'add','index','del','edit','destroy','restore','multi','recyclebin';
            //表注释
            $tableComment = $modelTableInfo['Comment'];
            $tableComment = mb_substr($tableComment, -1) == '表' ? mb_substr($tableComment, 0, -1) . '接口' : $tableComment;
            $apiAddDoc = [
                0=>"\n\t/**\n\t* 新增".$tableComment,
                1=>"* @ApiTitle    (新增{$tableComment})",
                2=>"* @ApiSummary  (新增{$tableComment})",
                3=>"* @ApiMethod   (POST)",
                4=>"* @ApiRoute    (/api/".strtolower($controllerName)."/add)",
                5=>"* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiIndexDoc = [
                "* ".$modelTableInfo['Comment']."列表接口",
                "* @ApiSummary  (新增".$modelTableInfo['Comment']."列表接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/index)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiDelDoc = [
                "* ".$modelTableInfo['Comment']."删除接口",
                "* @ApiSummary  (新增".$modelTableInfo['Comment']."删除接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/del)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiEditDoc = [
                "* ".$modelTableInfo['Comment']."编辑接口",
                "* @ApiSummary  (".$modelTableInfo['Comment']."编辑接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/edit)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiDestroyDoc = [
                "* 新增".$modelTableInfo['Comment']."真实删除接口",
                "* @ApiSummary  (".$modelTableInfo['Comment']."真实删除接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/destroy)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiRestoreDoc = [
                "* ".$modelTableInfo['Comment']."恢复接口",
                "* @ApiSummary  (".$modelTableInfo['Comment']."恢复接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/restore)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiMultiDoc = [
                "* ".$modelTableInfo['Comment']."批量更新接口",
                "* @ApiSummary  (新增".$modelTableInfo['Comment']."批量更新接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/multi)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            $apiRecyclebinDoc = [
                "* ".$modelTableInfo['Comment']."回收站接口",
                "* @ApiSummary  (".$modelTableInfo['Comment']."回收站接口)",
                "* @ApiMethod   (POST)",
                "* @ApiRoute    (/api/".strtolower($controllerName)."/recyclebin)",
                "* @ApiHeaders  (name=token, type=string, required=true, description=\"请求的Token\")"
            ];

            //循环所有字段,开始构造视图的HTML和JS信息
            foreach ($columnList as $k => $v) {
//                * @ApiParams   (name="password", type="file", required=true, description="密码")
//                * @ApiReturnParams   (name="id", type="string", required=true, sample="用户id")
//                * @ApiReturn   ()
                //新增接口文档组装
                $apiAddDoc[$k+6] = "* @ApiParams   (name=\"{$v['COLUMN_NAME']}\", type=\"{$v['DATA_TYPE']}\", required=true, description=\"{$v['COLUMN_COMMENT']}\")";//
                $apiAddDoc[$k+6+count($columnList)] = "* @ApiReturnParams   (name=\"{$v['COLUMN_NAME']}\", type=\"{$v['DATA_TYPE']}\", required=true,sample=\"{$v['COLUMN_COMMENT']}\", description=\"{$v['COLUMN_COMMENT']}\")";//
            ...
            $apiAddDoc[count($apiAddDoc)+1] = "* @ApiReturn   ()\n\t*/";//
            //循环关联表,追加语言包和JS列
            foreach ($relations as $index => $relation) {
            ...
            if ($relations) {
                ...
                foreach ($relations as $index => $relation) {
                    ...
                    if($relationForeignKey == 'user_id'){
                        $data['extendFields'] = "        \$this->extendFields = [
...
EOD;
    ...
    {
        return __DIR__ . DS . 'ApiCrud' . DS . 'stubs' . DS . $name . '.stub';
    }
